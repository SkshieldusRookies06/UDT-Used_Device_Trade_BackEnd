...
-- 시각은 CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00') 로 KST 벽시계를 명시한다 — NOW() 는 DB 서버 시간대를 타서
-- 팀원 PC의 MariaDB 시간대가 UTC 면 그 사람 화면에서만 9시간 어긋난다 (hibernate.jdbc.time_zone: Asia/Seoul 과 맞춘다).
...
INSERT INTO users (email, password, nickname, role, balance_krw, created_at) VALUES
  ('admin@udt.test',  '$2a$10$7B30C.Z.4zQsuf7b9xrG3uwsv6exRv5C2CUBwDhXBIKVaTb7IJ3Uq',  '관리자',   'ADMIN',  0,       CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')),
  ('seller1@udt.test','$2a$10$i01YpDmayKZMyujtk7a05edx5zza.eJGkemA.3JgCdyR3qtmdHSJO', '판매왕',   'MEMBER', 2520000, CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')),
  ('buyer1@udt.test', '$2a$10$i01YpDmayKZMyujtk7a05edx5zza.eJGkemA.3JgCdyR3qtmdHSJO', '구매자',   'MEMBER', 2000000, CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')),
  ('buyer2@udt.test', '$2a$10$i01YpDmayKZMyujtk7a05edx5zza.eJGkemA.3JgCdyR3qtmdHSJO', '알뜰구매', 'MEMBER', 300000,  CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00'));

INSERT INTO categories (name) VALUES
  ('노트북'), ('스마트폰'), ('태블릿'), ('이어폰'), ('기타');

INSERT INTO products (seller_id, category_id, title, description, price_krw, condition_grade, status, created_at, updated_at)
SELECT u.id, c.id, t.title, t.description, t.price_krw, t.condition_grade, t.status, CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00'), CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')
FROM (
...
INSERT INTO products (seller_id, category_id, title, description, price_krw, condition_grade, status, created_at, updated_at)
SELECT u.id, c.id, t.title, t.description, t.price_krw, 'A', t.status, CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00'), CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')
FROM (
...
SELECT p.id, u.id, p.price_krw, 'PAID', NULL, NULL, NULL, CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')
FROM products p JOIN users u ON u.email = 'buyer1@udt.test' WHERE p.title = '아이맥 24 M1';

INSERT INTO transactions (product_id, buyer_id, amount_krw, status, courier, tracking_no, confirmed_at, created_at)
SELECT p.id, u.id, p.price_krw, 'SHIPPING', 'CJ대한통운', '123456789012', NULL, CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')
FROM products p JOIN users u ON u.email = 'buyer1@udt.test' WHERE p.title = '델 XPS 13 2023';

INSERT INTO transactions (product_id, buyer_id, amount_krw, status, courier, tracking_no, confirmed_at, created_at)
SELECT p.id, u.id, p.price_krw, 'CONFIRMED', 'CJ대한통운', '123456789013', CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00'), CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')
FROM products p JOIN users u ON u.email = 'buyer1@udt.test' WHERE p.title = '아이패드 미니 6';

INSERT INTO transactions (product_id, buyer_id, amount_krw, status, courier, tracking_no, confirmed_at, created_at)
SELECT p.id, u.id, p.price_krw, 'DISPUTED', 'CJ대한통운', '123456789014', NULL, CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')
...
SELECT tx.id, tx.buyer_id, '수령한 제품 액정에 멍이 있습니다. 사진 첨부합니다.', 'OPEN', NULL, NULL, CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')
...
SELECT u.id, p.id, CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+09:00')
