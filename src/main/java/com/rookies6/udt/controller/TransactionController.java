...
public class TransactionController {
    ...
    public TransactionResponse purchase(@RequestParam Long buyerId, @PathVariable Long productId) {
        return transactionService.purchase(buyerId, productId);
    }

    // BR-04 송장 입력
    @PostMapping("/api/transactions/{transactionId}/shipping")
    public TransactionResponse registerShipping(@RequestParam Long sellerId,
                                                @PathVariable Long transactionId,
                                                @Valid @RequestBody ShippingRequest request) {
        return transactionService.registerShipping(sellerId, transactionId, request);
    }

    // BR-05 구매 확정
    @PostMapping("/api/transactions/{transactionId}/confirm")
    public TransactionResponse confirm(@RequestParam Long buyerId, @PathVariable Long transactionId) {
        return transactionService.confirm(buyerId, transactionId);
    }

    // BR-07 관리자 강제 환불
    @PostMapping("/api/admin/transactions/{transactionId}/force-refund")
    public TransactionResponse forceRefund(@PathVariable Long transactionId) {
        return transactionService.forceRefund(transactionId);
    }

    // BR-08 관리자 강제 확정
    @PostMapping("/api/admin/transactions/{transactionId}/force-confirm")
    public TransactionResponse forceConfirm(@PathVariable Long transactionId) {
