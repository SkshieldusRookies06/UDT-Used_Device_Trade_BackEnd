...
```
1) 터미널 1 — (cd backend && ./mvnw spring-boot:run -Dspring-boot.run.profiles=local)
...
```
5) (cd backend && ./mvnw test) 에러 0 · node seams/check-api.mjs OK
...
**30분 넘으면 `<백엔드 오너 이름>`에게 말한다. 규약 4번이고, 감점이 아니다.**
